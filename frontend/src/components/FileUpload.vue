...
const selectedKb = ref(
  currentKb.value.id
    ? { ...currentKb.value }
    : { id: null, name: '不分配到知识库', kb_type: 'analysis' },
)
...
<template>
  <div class="max-w-2xl mx-auto">
    ...
    <details class="upload-options">
      ...
      <div
        v-if="isStorageKb"
        class="bg-white rounded-xl p-5 border border-slate-200 mb-4"
      >
        <label class="block text-sm font-medium text-slate-700 mb-3"
          >切片方式</label
        >
        <div
          class="flex items-center gap-3 p-4 rounded-xl border border-amber-200 bg-amber-50 text-amber-700 text-sm"
        >
      ...
      <div v-else class="bg-white rounded-xl p-5 border border-slate-200 mb-4">
        <label class="block text-sm font-medium text-slate-700 mb-3"
          >切片方式</label
        >
        <p v-if="sliceTypesLoading" class="text-sm text-slate-500">
          加载切片方式...
        </p>
        <p v-else-if="sliceTypesError" class="text-sm text-red-500">
          {{ sliceTypesError }}
        </p>
        <div v-else class="space-y-3">
          ...
          >
            <input
              ...
              v-model="sliceType"
              :value="type.value"
              class="mt-1 w-4 h-4 text-purple-500"
            />
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-1">
    ...
    >
      ...
      <label
        class="inline-block px-4 py-2 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 cursor-pointer transition-colors"
      >
        选择文件
    ...
    <div
      v-if="files.length > 0"
      class="mt-4 bg-white rounded-xl border border-slate-200 overflow-hidden"
    >
      <div
        class="p-4 border-b border-slate-100 flex items-center justify-between"
      >
        <span class="text-sm font-medium text-slate-700"
          >待上传文件 ({{ files.length }})</span
        >
        <button
          @click="files = []"
      ...
      <ul class="divide-y divide-slate-100">
        ...
        >
          <div class="flex items-center gap-3 min-w-0">
            <div
              class="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center shrink-0"
            >
