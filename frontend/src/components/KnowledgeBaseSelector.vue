...
<template>
  <Teleport to="body">
    ...
    >
      ...
      >
        <button
          class="plain-button absolute top-4 right-4"
          aria-label="关闭知识库选择"
          @click="emit('close')"
        >
          ✕
        </button>
        <h3 class="text-lg font-semibold text-slate-800 mb-4">选择知识库</h3>
        ...
        <div class="min-h-50 max-h-100 overflow-y-auto">
          <div v-if="loading" class="text-center p-8">加载中…</div>
          <ul v-else class="divide-y divide-slate-100">
            <li
              v-for="kb in childrenKbs"
              :key="kb.id"
              class="p-3 flex justify-between items-center group"
            >
              ...
              <div class="flex items-center gap-2">
                ...
                <button
                  @click="navigate(kb.id)"
                  class="text-sm text-slate-500 hover:underline opacity-50 group-hover:opacity-100"
                >
                  展开
                </button>
              </div>
            ...
            <li
              v-if="!loading && childrenKbs.length === 0"
              ...
              selectKb(
                currentParent
                  ? {
                      id: currentParent,
                      name:
                        breadcrumbs[breadcrumbs.length - 1]?.name || '根目录',
                      kb_type: currentKbInfo?.kb_type,
                    }
                  : null,
              )
            "
