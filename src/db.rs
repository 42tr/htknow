...
pub async fn init() -> anyhow::Result<SqlitePool> {
    ...
    ensure_file_contents_table(&pool).await?;
    ensure_content_id_columns(&pool).await?;
    migrate_content_data(&pool).await?;
...
async fn ensure_file_contents_table(pool: &SqlitePool) -> anyhow::Result<()> {
    sqlx::query(
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_file_contents_hash ON file_contents(hash)")
        .execute(pool)
        .await?;
...
async fn ensure_content_id_columns(pool: &SqlitePool) -> anyhow::Result<()> {
    let files_columns = sqlx::query("PRAGMA table_info(files);").fetch_all(pool).await?;
    if !files_columns.iter().any(|row| row.get::<String, _>("name") == "content_id") {
        sqlx::query("ALTER TABLE files ADD COLUMN content_id INTEGER").execute(pool).await?;
        info!("Added content_id column to files table");
    }

    let slices_columns = sqlx::query("PRAGMA table_info(slices);").fetch_all(pool).await?;
    if !slices_columns.iter().any(|row| row.get::<String, _>("name") == "content_id") {
        sqlx::query("ALTER TABLE slices ADD COLUMN content_id INTEGER").execute(pool).await?;
        info!("Added content_id column to slices table");
    }

    let pdf_columns = sqlx::query("PRAGMA table_info(pdf_contents);").fetch_all(pool).await?;
    if !pdf_columns.iter().any(|row| row.get::<String, _>("name") == "content_id") {
        sqlx::query("ALTER TABLE pdf_contents ADD COLUMN content_id INTEGER").execute(pool).await?;
        info!("Added content_id column to pdf_contents table");
    }

    sqlx::query("CREATE INDEX IF NOT EXISTS idx_files_content_id ON files(content_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_slices_content_id ON slices(content_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_pdf_contents_content_id ON pdf_contents(content_id)")
        .execute(pool)
        .await?;
...
async fn migrate_content_data(pool: &SqlitePool) -> anyhow::Result<()> {
    let pending_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM files WHERE status = 1 AND content_id IS NULL",
    )
    .fetch_one(pool)
    .await?;
    ...
    info!("Migrating {} existing files to file_contents model", pending_count);

    const BATCH_SIZE: i64 = 500;
    let mut migrated = 0i64;

    loop {
        let files: Vec<(i64, String, Option<String>)> = sqlx::query_as(
            "SELECT id, hash, content FROM files \
             WHERE status = 1 AND content_id IS NULL \
             LIMIT ?",
        )
        .bind(BATCH_SIZE)
        .fetch_all(pool)
        ...
        for (file_id, hash, content) in files {
        ...
        info!("Migrated {}/{} files to file_contents", migrated, pending_count);
    }

    let slices_updated = sqlx::query(
        "UPDATE slices \
         SET content_id = COALESCE(\
             (SELECT f.content_id FROM files f WHERE f.id = slices.file_id),\
             file_id\
         )\
    ...
    if slices_updated > 0 {
        info!("Backfilled content_id for {} slices", slices_updated);
    }

    let pdf_updated = sqlx::query(
        "UPDATE pdf_contents \
         SET content_id = COALESCE(\
             (SELECT f.content_id FROM files f WHERE f.id = pdf_contents.file_id),\
             file_id\
         )\
         WHERE content_id IS NULL",
    )
    ...
    if pdf_updated > 0 {
        info!("Backfilled content_id for {} pdf_contents", pdf_updated);
    }

    info!("Content data migration completed: {} files migrated", migrated);
    Ok(())
}
